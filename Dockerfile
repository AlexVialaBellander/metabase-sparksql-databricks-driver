...
RUN clojure \
    -Sdeps "{:aliases {:relferreira_brigad_databricks {:extra-deps {com.metabase/sparksql-databricks {:local/root \"/build/driver\"}}}}}" \
    -X:build:relferreira_brigad_databricks \
    build-drivers.build-driver/build-driver! \
    "{:driver :relferreira_brigad_databricks, :project-dir \"/build/driver\", :target-dir \"/build/driver/target\"}"

# We create an export stage to make it easy to export the driver
FROM scratch as stg_export
COPY --from=stg_build /build/driver/target/relferreira_brigad_databricks.metabase-driver.jar /
...
COPY --chown=2000:2000 --from=stg_build \
    /build/driver/target/relferreira_brigad_databricks.metabase-driver.jar \
